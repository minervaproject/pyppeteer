...
def current_platform() -> str:
    ...
    if sys.platform.startswith('linux') and platform.uname().machine == "aarch64":
        return 'arm_linux'
...
def download_chromium() -> None:
    ...
    if current_platform() == 'arm_linux':
        exec_path = chromium_executable()
        if not exec_path.exists():
            raise Exception("Please install chromium via your package manager, eg 'sudo apt-get install chromium'")
    else:
